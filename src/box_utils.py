...
def assign_gts_to_anchors_or_proposals(
    gt_boxes,
    gt_masks,
    anchors,
    n_sample,
    ...
    distributed_gt_boxes = torch.zeros((len(anchors), 4), dtype=torch.float32).to(device)
    distributed_gt_masks = torch.zeros(
        (len(anchors), 28, 28), 
        dtype=torch.float32
    ).to(device)
    ...
    if len(gt_boxes) == 0:
        ...
        return labels, distributed_gt_boxes, distributed_gt_masks, distributed_cls_indexes

    ious = box_iou(anchors, gt_boxes)
    max_iou_anchor_index = torch.argmax(ious, dim=0)  # return k anchor indexes corresponding to k gts
    labels[max_iou_anchor_index] = 1
    # assign max iou target to as positve to avoid null positive samples
    distributed_gt_boxes[max_iou_anchor_index] = gt_boxes
    distributed_gt_masks[max_iou_anchor_index] = gt_masks
    ...
    distributed_gt_boxes[pos_index] = gt_boxes[max_iou_target_idx_per_anchor[pos_index]]
    distributed_gt_masks[pos_index] = gt_masks[max_iou_target_idx_per_anchor[pos_index]]
